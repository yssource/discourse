  ...
  context "rate limiting" do
    ...
    describe "diagnostic information" do
      ...
      it "is included when the requests-per-minute limit is reached" do
        ...
        expect(status).to eq(429)
        expect(x).to eq(1)
        expect(headers["Discourse-Rate-Limit-Error-Code"]).to eq("ip_60_mins_limit")
        expect(response.first).to include("Error code: ip_60_mins_limit.")
      end
    ...
    it "users with high enough trust level are not rate limited per ip" do
      ...
      envs.each do |env|
        ...
        status, headers, response = middleware.call(env)
        expect(status).to eq(429)
        expect(headers["Discourse-Rate-Limit-Error-Code"]).to eq("id_60_mins_limit")
        expect(response.first).to include("Error code: id_60_mins_limit.")
      end
      expect(x).to eq(3)
    end
  end
